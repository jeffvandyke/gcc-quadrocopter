function [quadc,history] = simulate_quad_testPITCH()
...
if imag(quadc.x)
    ...
    n = n/sqrt(n(1)^2+n(2)^2+n(3)^2); % n is the normal vector of the quad
    quadc.F = -n;
...
%% only plot the quadrotor every few calculations in order to increase speed
...
%start video object thing
    ...
    %Quad physical properties*      * not perfectly accurate. I didn't
    %                                   measure, only guessed =/   !!!
    ...
    quadc.desiredTip = [0,0,1]; % || to ground
   
    quadc.rangePitchPID = 5;
    quadc.alt = quadc.z;
...
if abs(pitchinc) > quadc.rangePitchPID
    pitchinc = sign(pitchinc)*quadc.rangePitchPID;
end
...
if abs(rollinc) > quadc.rangePitchPID
    rollinc = sign(rollinc)*quadc.rangePitchPID;
end
