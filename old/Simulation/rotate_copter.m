...
% Rotates the goliath matrix containing all all points for the copter.
% the goliath matrix is a nx3 matrix containing all the points of the
% quadrotor as constructed by the '.m' file [ build_quad() ] Goliath is
% stored into a .mat file named 'goliath_xy.mat'
%
% Also plots the quad
...
if isempty(origGoliath)
    load('goliath_xy.mat');
    origGoliath = [goliath, ones(length(goliath),1)];
    clear goliath
end
...
if imag(roll)
       display('Rotate broke!')
       pause;
       return
end

view_copter(goliath,x,y,z,roll,pitch,yaw);   %less on the plot for speed

end
function [] = view_copter(goliath,x,y,z,roll,pitch,yaw)
%plots copter in the given position (x,y,z) and given rotation (roll,pitch,yaw)
 % needs to already have been rotated.
 
%% split HUGE matrix back up
% not sure what the values in SQR are, but they are needed for the
% cylinders (props and motors)
...
% breaks the huge matrix of data points into parts of the quad for plotting
...
%% plotting
cla % clear the figure to keep only one quadrotor plotted at a time

hold on  % hold on to plot the entire quad on the figure
      
    plot3(0,0,0,'+','MarkerEdgeColor','r','MarkerSize',15) % put a red '+' at the origin
    
    % plot the top middle, bottom Black
    patch(top(:,1),top(:,2),top(:,3),'k');
    patch(middle(:,1),middle(:,2),middle(:,3),'k')
    patch(bottom(:,1),bottom(:,2),bottom(:,3),'k')
    
    %plot wing1 and wing2 Green
    ...
    %plot motors 1-4 the color specified at the beginning of this function
    ...
    %plot props 1-4 the color specified at the beginning of this function
    ...
    % keep the perspective of the viewer the same. or change this to vary
    %  it up
        axis equal
