function [quadc,history] = simulate_quad_testRuns(rkp,rki,rkd)


[quadc,history] = initialize(rkp,rki,rkd);
...
function [quadc, history] = initialize(rkp,rki,rkd)
%% Initial Setting up (ignore)
    ...
    quadc.pwm4 = 127;
        ...
        quadc.ya = 0;
        quadc.yi = 0;
    quadc.z  = 1; % start flying in the air.
        quadc.zv = 0;
        quadc.za = 0;
        quadc.zi = 0;
    ...
    history.zd = abs(quadc.z - quadc.alt);
    history.xd = abs(quadc.x);
    history.yd = abs(quadc.y);
    history.rolld   = abs(quadc.roll - xyz2roll(quadc.desiredTip));
    history.pitchd  = abs(quadc.pitch - xyz2pitch(quadc.desiredTip));
    history.yawd    = abs(quadc.yaw); % - desiredYaw;
...
%% Height ONLY
quadc.zd = quadc.alt - quadc.z;




zinc = quadc.zd*history.zKP - quadc.zv*history.zKD - history.zKI*quadc.zi;
...
function history = record(quadc,history)
    ...
    history.zd = history.zd + abs(quadc.z - quadc.alt);
    history.xd = history.xd + abs(quadc.x);
    history.yd = history.yd + abs(quadc.y);
    history.rolld   = history.rolld + abs(quadc.roll - xyz2roll(quadc.desiredTip));
    history.pitchd  = history.pitchd + abs(quadc.pitch - xyz2pitch(quadc.desiredTip));
    history.yawd    = history.yawd + abs(quadc.yaw); % - desiredYaw;
end
